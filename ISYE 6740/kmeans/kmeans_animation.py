...
if iscomparebruteforce == 1:
    ...
    x = np.concatenate(
        (x, np.random.randn(dim, m) + np.tile(np.array([1, 2]), (m, 1)).T), axis=1
    )
    ...
    for i in range(1, m):
        ...
        for j in partition1:
            ...
            obj = obj + np.sum(
                np.sum(
                    np.power(
                        x[:, group1_idx] - np.tile(center1, (group1_idx.shape[0], 1)).T,
                        2,
                    )
                )
            )
...
newobj = np.sum(
    np.sum(np.power(x[:, idx1] - np.tile(center1, (idx1.shape[0], 1)).T, 2))
