...
class Graph:
    ...
    def add_edge(self, source: str, target: str) -> None:
        ...
        """
        combos = tuple(itertools.permutations((source, target)))

        if combos[0] not in self.edges and combos[1] not in self.edges:
    ...
    def max_degree_nodes(self) -> dict:
        ...
        counts = dict(collections.Counter([g for e in self.edges for g in e]))
...
class TMDBAPIUtils:
    ...
    def get_movie_cast(
        ...
        response = requests.get(url).json()["cast"]

        exclude_ids = exclude_ids or []

        output_dict = [e for e in response if e["id"] not in exclude_ids]

        if limit:

            order_ids = [i for i in range(limit)]

            output_dict = [e for e in output_dict if e["order"] in order_ids]
    ...
    def get_movie_credits_for_person(
        ...
        credit_dict = requests.get(url).json()["cast"]
        ...
        if start_date:
            credit_dict = [
                e
                for e in credit_dict
                if e.get("release_date") and e["release_date"] >= start_date
            ]
        ...
        if end_date:
            credit_dict = [
                e
                for e in credit_dict
                if e.get("release_date") and e["release_date"] <= end_date
            ]
...
def build_network(
    initial_id, initial_name, network, start="1999-01-01", end="1999-12-31"
):
    tmdb_api_utils = TMDBAPIUtils(api_key="d65edc095fb13e40afde9aa397f8d530")
    if "," in initial_name:
        initial_name = initial_name.replace(",", " ")

    network.add_node(initial_id, initial_name)

    visited = set([initial_id])
    ...
    for _ in range(3):
        ...
        for _ in range(layer_size):
            ...
            actor_movies = [
                movie["id"]
                for movie in tmdb_api_utils.get_movie_credits_for_person(
                    actor_id, start_date=start, end_date=end
                )
            ]

            for film_id in actor_movies:
                try:
                    cast = tmdb_api_utils.get_movie_cast(
                        film_id, limit=5, exclude_ids=[actor_id]
                    )
                except:
                    continue

                for e in cast:
                    co_id, co_name = str(e["id"]), e["name"]
                    if "," in co_name:
                        co_name = co_name.replace(",", " ")
                    if co_id not in network.nodes:
                        network.add_node(co_id, co_name)
                    if (actor_id, co_id) not in network.edges and (actor_id != co_id):
                        network.add_edge(actor_id, co_id)
...
if __name__ == "__main__":
    ...
    build_network("2975", "Laurence Fishburne", graph)
